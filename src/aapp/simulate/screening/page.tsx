        ...
        <main className="min-h-screen bg-slate-50 text-slate-900">
            <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
                <header className="mb-6 flex items-center justify-between rounded-[28px] border bg-white p-4">
                    ...
                    <Link href={`/upload?${new URLSearchParams({ role }).toString()}`} className="rounded-full border px-4 py-2 text-sm">
                        Zurueck
                    </Link>
                </header>

                <section className={`rounded-[28px] border p-6 sm:p-8 ${passed ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}>
